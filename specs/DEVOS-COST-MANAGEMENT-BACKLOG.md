...
| Cost-aware agent selection | Agent Framework §9 (Agent Selection Algorithm: *"Apply optional performance/cost preferences"*); Selection Criteria table (*"Cost: Budget tier"*) | `apps/worker/src/agent-task-router.ts` still routes by a hardcoded seeded `agentRef`, not by role, capability, or any performance/cost preference — a pre-existing, already-disclosed limitation (DEVOS-038's own verification-debt entry, carried forward unchanged through every sprint since). There is no selection *algorithm* today for a cost preference to attach to. |
...
- **Cost-aware agent selection / model routing** (Agent Framework §9/§43) — real scoping needs a real agent-selection algorithm to extend, and `apps/worker/src/agent-task-router.ts` does not have one (DEVOS-038's long-standing disclosed debt: routing is by hardcoded seeded `agentRef`, not role or capability). Building a real selection algorithm is itself a separate, non-trivial precondition task before a cost preference could meaningfully attach to it.

This document will be revised with a real Sprint 19 backlog (grounded the same way §2 grounds Sprints 17–18) once Sprint 18 is complete and either precondition above is ready to be tackled — or sooner, if you want one of the two preconditions scoped as its own task before that.
...
- **Cost-aware agent selection / model routing** (Agent Framework §9/§43) — `apps/worker/src/agent-task-router.ts` has no real selection algorithm yet (DEVOS-038's long-standing disclosed debt); there is nothing for a cost preference to attach to until that routing algorithm itself is built, which is out of Sprint 17–18's own scope.

...
import { runTechnicalDesignAgentTask } from '../src/tasks/run-technical-design-agent-task.js';
...
const schemas: SchemaRepository = {
  resolve: async () => ({
    ...
    version: 1,
    fields: { summary: { type: 'string' }, decisions: { type: 'array' } },
  }),
};
...
function buildScenario() {
  ...
  let projectArtifacts: Artifact[] = [prdArtifact];
  ...
  const artifactVersions: ArtifactVersionRepository = {
    getById: async (id) => (id === prdVersion.id ? prdVersion : null),
    listForArtifact: async (artifactId) => (artifactId === prdArtifact.id ? [prdVersion] : []),
    create: async () => {},
  };
...
describe('runTechnicalDesignAgentTask', () => {
  let storageDir: string;

  beforeEach(async () => {
    storageDir = await mkdtemp(path.join(tmpdir(), 'devos-run-technical-design-agent-task-'));
  });
  ...
  it('produces a schema-validated technical design artifact linked to the PRD via provenance and context manifest', async () => {
    ...
    const output = await runTechnicalDesignAgentTask(deps, scenario.task);
    ...
    expect(publishedVersion?.metadata).toMatchObject({
      ...
      derivedFromArtifactVersionId: scenario.prdVersion.id,
      decisions: ['Wrap the query executor with a timing decorator.'],
    });
    ...
    expect(scenario.contextManifests[0]?.sources).toContainEqual(
      expect.objectContaining({
        type: 'ARTIFACT',
        ref: `artifact:${scenario.prdArtifact.id}:v${scenario.prdVersion.version}`,
  ...
  it('throws when no PRD exists for the run', async () => {
    const scenario = buildScenario();
    scenario.removePrdArtifact();
    const modelAdapter: AgentModelAdapter = {
    ...
    await expect(runTechnicalDesignAgentTask(deps, scenario.task)).rejects.toThrow(
      'No PRD artifact found',
    );
  });
});

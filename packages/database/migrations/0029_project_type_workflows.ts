...
export async function up(db: Kysely<any>): Promise<void> {
  await db.schema
    .createTable('project_type_workflows')
    ...
    .addUniqueConstraint('project_type_workflows_project_type_id_key_key', [
      'project_type_id',
      'key',
    ])
    .execute();
  ...
  await db.schema
    .createIndex('project_type_workflows_project_type_id_idx')
    .on('project_type_workflows')
    .column('project_type_id')
    .execute();
}

export async function down(db: Kysely<any>): Promise<void> {
  await db.schema.dropTable('project_type_workflows').execute();
}
